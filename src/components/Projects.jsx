    ...
    <section className="bg-white p-4 sm:p-6 rounded-xl shadow-md hover:shadow-xl transition-all">
      ...
      <div className="space-y-4 sm:space-y-6">
        {projects.map((project, index) => (
          <div key={index} className="project-card p-4 sm:p-5 rounded-lg border border-gray-100 bg-white">
            ...
            <div className="mt-3 sm:mt-4 flex flex-wrap gap-2">
              {project.tags.map((tag, idx) => (
                <span key={idx} className={`text-xs px-2 py-1 rounded`}>{tag}</span>
              ))}
            </div>
            <div className="mt-3 sm:mt-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
              <a href={project.liveDemo} target="_blank" className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition flex items-center text-sm sm:text-base">
